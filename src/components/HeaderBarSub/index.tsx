...
const HeaderBarSub = React.memo((props: HeaderBarProps) => {
  ...
  const clickFavorite = () => {
    if (!serviceId) return;
    const res = localStorage.getItem("@ecyeh_favorite");
    if (!res) {
      localStorage.setItem("@ecyeh_favorite", JSON.stringify([serviceId]));
      setIsFavorite(true);
        ...
        <Toolbar className={classes.containerHeader}>
          {isGoBack ? (
            <IconButton
              ...
              edge="start"
              className={clsx(
                { [classes.fontSmall]: login },
                classes.menuButton
              )}
              color="inherit"
              aria-label="menu"
            >
              {login ? translate("SIGN_IN") : <KeyboardBackspaceIcon />}
            </IconButton>
          ) : (
            ""
          )}
          ...
          >
            {name ? <p>{name}</p> : "Finding Your Way in PA"}
          </Typography>
